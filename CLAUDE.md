...
**但那道闸有个没覆盖到的格子**（2026-09-20，`15_turnover_sweep.py` 重跑发现）：
它比的是「间隔 1 vs 间隔 10」，**两边 `keep_rank` 同值** —— 从没比过
「日频 + 高迟滞」。在 `bias20` 上那一格是全表最好的：换手 0.116（比间隔10
的 0.078 高，但比间隔5 的 0.145 低）、净年化 +34.43%、而**相位极差是 0**
（日频每天调仓，没有"从哪天开始"这个自由度）。拉长间隔的格子极差普遍
**大于**它们跑赢基准的幅度。
这只在 `bias20` 因子上测过，**不构成改配置的依据**；要动就在生产模型上
补跑那一格再进八项闸。
