...
**关掉择时（`QBG_MARKET_SMA=0`）的闸注定过不了 `drawdown` 那一项** ——
关掉择时就是拿回撤换收益，而八项闸明写着回撤不许恶化。**不要去调闸的阈值
让它过。** 八项闸是给参数微调设计的；关掉一整个风控组件是**风险偏好变更**，
闸给不出答案，只能把价码算准（`scripts/20_timing_gate.py`）。
两条臂给出相反答案时以**和实盘同口径**的那条为准：等权全池说该关，
k=3 说回撤会翻倍（-16.14% → -32.05%），而实盘是 k=3。
