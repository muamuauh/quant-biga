...
**⚠ `stop_loss_pct`(8%) 从来没有接线**（2026-09-14 查明）：它在 yaml 里，
`src/qbg` 里**零引用**，只在 `scripts/20_timing_gate.py` 的一句 `print` 里出现过。
此前本文件和 `docs/p3-model-validation.md` 都写着"单票由 8% 止损管" —— 那是错的，
**关择时的决定当时依据的下行保护里，有一半并不存在**。要不要接线由操作者决定。
...
10/10 年化 −3.7 点、四个子区间全部跑输。短窗口那次是运气，而且看着非常像真的
（16 组参数全部变好）。**评估任何参数都要跑两个窗口**（`--experiment cn_lgb_mid`）。

两条接线纪律：**只在监控日强卖**（调仓日交给选股）；**止盈卖出不写调仓日期**
（写了会重置调仓周期）。持仓降级时既不刷新峰值也不强卖。

**调仓间隔已改为 10 日**（2026-09-14 操作者改 .env）：`12_rebalance_gate.py`
修掉三处漏传（相位 / keep_rank / 滑点，每一处都单向偏向日频）之后，日频 vs 10 日
八项全过、子区间 4/4。**复核这个决定要传 `--baseline 1`**，否则现行配置已是 10，
默认会变成自己比自己。
