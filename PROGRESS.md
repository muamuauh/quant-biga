...
**P9a 与 P9b 都已验完**（2026-08-24 买入侧 + 08-25 卖出侧，见下）。
剩下的是把它接进日流程：

1. `daily_cycle` 现在**写死 `AdvisoryAdapter()`** —— 和 P9a 之前写死
   `ManualSource()` 是同一类问题。改成按 `settings.qbg_mode` 选：
   ADVISORY → `AdvisoryAdapter`，PAPER/LIVE → `EasytraderAdapter`。
2. 日报加「计划 vs 实际委托」对账段（`plans` 表已有，缺的是把回读到的
   `entrust_no` 和成交状态写进去）。
3. 接完先在 `QBG_MODE=PAPER` 下空跑几天，再谈三把锁。
...
| P9a 只读持仓 | ✅ | 已接入 daily_cycle；取值与 T+1 语义两日实测通过 |
| P9b 下单 | ✅ | 买入/卖出双向在模拟盘端到端跑通并撤单 |
| P9c 编排接入 | ⬜ | daily_cycle 仍写死 AdvisoryAdapter |
...
### 账户守卫 + P9c + 日报成本段（2026-08-25）

**账户守卫。** `QBG_MODE=PAPER` 在三把锁那里是直接放行的，但 PAPER 只是
`.env` 里的一句声明 —— 代码无从知道客户端登录的是模拟盘还是真钱账户。
客户端登真钱 + `.env` 写 PAPER 就会在真钱上下单且一道锁都不查。
新增 `assert_account_matches_mode`，用客户端的资金账号（control 2322）作独立信源：
PAPER 要求账户名含 `qbg_paper_account_pattern`（默认「模拟」），LIVE 反向拒绝模拟盘，
**读不到账户名一律拒绝下单**。

**P9c 编排接入。** `daily_cycle` 按 `QBG_MODE` 选适配器（此前写死 AdvisoryAdapter）。
顾问清单永远写；券商链路异常不掀翻日流程（否则连日报和邮件都没有）。
日报新增「计划 vs 实际委托」段，逐笔列合同编号或失败原因。

**清理测试持仓时又撞出两个真 bug**：

1. **成交了却报失败** —— 卖单实际「全部成交」（6220417823 @7.87），adapter 却因
   「提交后立刻读一次」而判定没下出去并中止整批。客户端刷新委托要走服务器往返。
   这个方向的误判最危险：以为没下成、实际下成了，上层重试就会**重复下单**。
   改成轮询 3 次 × 4 秒，并把「一次都没读成」和「读成了但没这笔」分开报。
2. **清仓当天 P9a 直接不可用** —— 卖光后持仓表仍留一行，代码名称都在但数量全归零，
   连报三条 FATAL 让 `load()` 抛异常。判据用「股数和市值同时为 0」，
   只看股数会误伤「股数>0 但市值读成 0」那种取表出错。

**多收件人** —— smtplib 对逗号本来就支持，但**分号会静默丢掉第二个人**（实测）。
两种分隔符都接受并去重。

**日报新增「今日成本」段**（用户要求）：佣金 / 印花税 / 过户费 分列，
标注卖出额与最低佣金触发笔数，给出 bp 和「要涨多少才打平」。
LLM 复核成本（calls / tokens / USD）同段列出。样例渲染里 1 万元的买单
自动被标出触及最低佣金 —— 实际费率翻倍到万 5，这正是小账户的隐藏代价。

**配置变更**（用户要求）：`QBG_AGENTS_CANDIDATES` 9→5，
`QBG_REBALANCE_EVERY_DAYS` 10→1（每日调仓，测试期）。
⚠️ 后者与 `config.py` 里「绝不要为了看起来更活跃调低它」的实测结论相反
（quant-trading：3000 美元账户日频调仓年化 −30%）。测试期合理，长期需回测复验。

**顺带修了一个坏测试**：`test_small_account_defaults` 断言的是全局 `settings`
（会读部署 `.env`），却自称在测默认值。因部署选择变红的测试只会逼人改测试
或忍受红灯。改成 `Settings(_env_file=None)`，和旁边的 `test_agents_default_off` 一致。

### P9 第二次验证：卖出路径 + T+1 解冻（2026-08-25 09:37）

**T+1 语义两日对照完成**，同一笔 601398 持仓：

| | 股票余额 | 可用余额 | 说明 |
|---|---:|---:|---|
| 08-24 当天买入 | 100 | **0** | T+1 冻结 |
| 08-25 隔夜 | 100 | **100** | 已解冻 |

这是 `sellable_qty` 唯一有意义的验证方式 —— 单看一天分不清「字段读对了」
和「恰好是 0」。它也是 easytrader 相对截图 OCR 的核心增量，T+1 闸全靠它。

**卖出链路端到端跑通**：

    ths.order.submit      code=601398 side=SELL qty=100 price=8.05
    ths.order.confirm_ok  卖出确认框解析并逐项核对通过
    ths.order.verified    entrust_no=6219942124
    ExecutionResult: ok=True submitted=1
    D 段撤单 → 备注「全部撤单」，当日成交为空，持仓完好

**卖单挂在市价上方 2%** 是刻意的：买单挂下方不成交，卖单同理要挂上方。
先前脚本写的是 `ref * 0.98`（下方 2%）—— 第一次跑因为必被 T+1 拒绝所以没暴露，
这次会**真的把持仓市价甩卖掉**。已改成 `ref * 1.02` 并写清理由：
我们验的是「委托能不能正确下出去并被回读到」，成交是券商撮合的事，
不归这条链路管。

至此 **P9a、P9b 的买入与卖出双向都已在模拟盘验完**，剩 P9c 编排接入。
