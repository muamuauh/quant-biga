...
三个计划任务，顺序是 **07:45 预检**（起 Clash + 同花顺）→ **08:00 盘前**
（拉数 + 重训 + LLM 逐票复核，**不下单**）→ **09:30 主流程**（读盘前结论 +
风控 + 下单）。开机后自动补跑。**本机（Windows 11）实测**，不是设想。
...
```
07:45  quant_biga_preflight   起 Clash + 同花顺、开代理/TUN   约 1 分钟
       ↓ 代理必须先起来：复核要调的 LLM 中转站走 Clash
08:00  quant_biga_premarket   拉数 + 滚动重训 + 逐票复核     约 1 小时
       ↓ data/reviews/<date>.json
09:30  quant_biga_daily       读缓存 + 风控 + 下单           约 2 分钟
```

**「过期不补跑」有上界也有下界。** 上界是"计划时间 + 窗口"，下界是"早于当天
计划时刻就算补跑"。下界是 2026-09-11 补的：`-ScheduledAt` 写进任务动作的只是
时刻、没有日期，所以前一天错过的那次今天一开机补上时，到达时间比当天的计划
时刻还**早**，上界根本判不出来。那次撞上 `session_guard` 才没造成后果，但
P8 自动复盘照跑，花了一次 LLM 调用 —— **早退不等于免费**。

**这个顺序是被依赖定死的。** 顺序排错了三个任务都还会跑，只是预检开代理时
复核已经失败、复核写缓存时日流程已经现场复核完了 —— **全程没有任何报错**。
所以 `setup_schedule.ps1` 在注册之前就校验 预检 < 盘前 < 主任务，不满足直接
退出。
...
三个任务的窗口不一样，因为各自的「还来得及吗」是不同的时刻：

| 任务 | 起跑 | 窗口 | 截止 | 为什么是这个时刻 |
|---|---|---|---|---|
| 预检 | 07:45 | 105 | 09:30 | 再晚就没有独立价值 —— `run_daily.ps1` 内部自己会跑一遍预检 |
| 盘前复核 | 08:00 | **30** | 08:30 | 复核要 58 分钟，08:30 起跑刚好赶在 09:30 之前写完缓存 |
| 日流程 | 09:30 | 120 | 11:30 | 上午收盘 |

盘前那 30 分钟看着苛刻，但**「跑不完」的代价不是零**：日流程照样退回现场
复核，同一批票的 LLM 账单等于付两遍。宁可干脆不跑。

（此前三个任务共用一个 `-WindowMinutes 120`，盘前因此拿到 120 分钟 ——
09:59 被唤起还会去跑一小时复核，而日流程早就退回现场复核了。）
...
`run_premarket.ps1` 内部**还会再跑一次预检**，那是**兜底**：预检任务被窗口闸
跳过、或者它自己失败的那天，复核不该跟着一起哑掉。预检幂等，重复跑没有副作用。

顺带一提，预检提前到 07:45 之后，人工登录同花顺的窗口从 15 分钟变成了
**105 分钟**（07:45 到 09:30）。
...
那天，晚上一开机 Windows 就把 07:45 的预检和 09:30 的日流程一起补跑 ——
...
另外，预检任务被单独叫起来时（07:45）此前**不留任何文件痕迹** —— 出事只剩
