...
| **easytrader + 同花顺客户端** | ✅ 可行（2026-08 复查后上调） | 见下方 §2.2.1。原文写的"久未维护"**已被证伪** |
| QMT / miniQMT (xtquant) | ❌ 门槛不够 | 见上 |

#### 2.2.1 同花顺自动化方案复查（2026-08-21）

初版计划（2026-08-10）写的「easytrader 久未维护、同花顺已难驱动」**是错的**，
基于二手社区帖子而非源码与仓库事实。2026-08-21 重新调研后更正如下。

**候选对比**（两者打的是同一个目标：同花顺的 `网上股票交易系统5.0`，
即 `xiadan.exe`；都用 pywinauto + 硬编码 control_id；都靠 Ctrl+A/Ctrl+C
把表格拷进剪贴板按 `\t` 切；都用 tesseract 识查询验证码）：

| | [easytrader](https://github.com/shidenggui/easytrader) | [THSAutoTrader](https://github.com/linlb/THSAutoTrader) |
|---|---|---|
| star / fork | **10071 / 3014** | 154 / 42 |
| 最近提交 | 2026-02-28 | 2026-03-31 |
| 许可 | MIT | Apache-2.0 |
| 分发 | **PyPI `easytrader` 0.23.7**（2025-04） | 无包、无 tag、无 release |
| 形态 | **Python 库**（进程内） | 独立 App：Flask + GUI 悬浮球 + 托盘 + WebRTC 信令 + HTTP 代理 |
| 取表格 | **三种策略**：`Copy` / `WMCopy`（`post_message`，不抢前台）/ `Xls`（走文件，不碰剪贴板） | 只有一种：强制 `click_input()` 抢焦点 + 剪贴板 |
| 下单 | `buy(code, price, amount)` — **限价** | `/xiadan?code=&status=&amount=` — **闪电买卖，无价格参数** |
| 其他后端 | **miniqmt 同一套 API** | 无 |

**选 easytrader，四条理由按重要性排**：

1. **THSAutoTrader 下不了限价单。** 这是硬伤不是偏好。它的 `/xiadan`
   只把代码敲进闪电下单框、按 21/23、填数量（control 1034）、点提交，
   **全程不碰价格框（control 1033）**，成交价是闪电下单框的默认对手价。
   而本项目 §5.1 / P4 的设计是**限价 + 涨跌停夹逼 + `order_slippage_pct` 让价**，
   接它等于把这一层扔掉。
2. **它把股票代码当键盘敲给"当前焦点窗口"**（`keyStr = code + ' ENTER 21 ENTER'`
   直接 `send_key`）。中途焦点一丢，这串数字会打到别处。easytrader 是
   `set_edit_text` 打进指定 control_id，不依赖焦点。
3. **仓库里躺着约 500 MB 无法审计的二进制**：`下单辅助程序.exe`（79 MB，
   预编译无源码）+ 完整 Tesseract 树，还提交了两份。再加上依赖 `openai`、
   WebRTC 信令服务器和一个 HTTP 反向代理 —— **一个能下单的进程里挂着远程控制通道**，
   与本项目"三把锁 + 只出站"的纪律形状相反。
4. **easytrader 的 `WMCopy` / `Xls` 能不抢鼠标键盘。** 挂定时任务时，
   一个每天固定时间抢前台焦点、模拟点击的东西，撞上人在用电脑就会出事。

**easytrader 的短板也要说清**：`pywinauto==0.6.6` 是 2019 年的死锁版本；
control_id 一样硬编码、同花顺大版本升级一样会崩；验证码要自己装 tesseract
（THSAutoTrader 是打包好的）。**它不比对方稳，只是形态更对。**

#### 2.2.2 本机实测（2026-08-21）

```
同花顺 9.60.61        D:\tonghuashun\同花顺\xiadan.exe   32-bit
qbg 环境              Python 3.11.15                     64-bit
xiadan.exe 完整性     High（以管理员身份运行）
easytrader            0.23.7 + pywinauto 0.6.6（已装进 qbg，无依赖降级）
```

**实测结论：跨完整性级别「读」是通的，「写」被挡住。** 按 hwnd 连接后：

| 动作 | 结果 |
|---|---|
| `WM_GETTEXT` 读窗口标题 | ✅ `'网上股票交易系统5.0'` |
| `Application().connect(handle=...)` | ✅ |
| 读控件 1012（资金余额） | ✅ 读到真实数值 |
| `main.descendants()` | ✅ 323 个子控件 |
| `Application().connect(path=)` / `connect(process=)` | ❌ `ProcessNotFoundError` |
| 模拟点击 / 按键 | ❌ pywinauto 警告 `no rights to make changes in the target GUI` |

**这个区分很重要，因为它是个陷阱**：能读到资金数字容易让人以为"通了"，
但 easytrader 取任何一张表都要先用菜单导航到对应查询页（点 `查询[F4]` →
`资金股票`），**那一步是写**。所以读得通 ≠ 能用。

另外两个实测发现：

* **`网上股票交易系统5.0` 窗口存在但 `visible=False` —— 是「精简模式」。**
  判据：`GetWindowPlacement` 的 `showCmd` 仍是 `SW_NORMAL(1)`、`IsIconic` 为假、
  但 `IsWindowVisible` 为假 —— 也就是程序主动 `ShowWindow(SW_HIDE)` 了主窗口，
  只留一个约 `238x29` 的下单小条。**这种状态下没有可操作的控件树**，
  UI 自动化完全无从下手（THSAutoTrader 的报错信息里也专门提了"不要进入精简模式"）。

  pywinauto 的 `find_windows()` 默认只返回可见窗口，这种情况下它报"找不到窗口"，
  会把人引向完全错误的方向。`tools/probe_ths.py` 因此改用裸 `EnumWindows` 自己找，
  并把**最小化 / 被程序隐藏(精简模式) / 窗口不存在(未登录)** 三种情况分开报——
  它们的修法完全不同。

* **两个进程别搞混**：行情主界面是 `hexin.exe`（标题 `同花顺(9.60.61) - 上证指数`），
  UI 自动化要的是**独立下单程序 `xiadan.exe`** 的 `网上股票交易系统5.0` 窗口。
  最大化前者对后者没有任何影响。
* **同一个 control_id 在窗口里出现多次**（1015/1016/1047 都命中 2 个元素，
  `ElementAmbiguousError`）。所以必须先导航到目标查询页再取控件，
  不能对着主窗口直接按 id 抓 —— 又回到"需要写权限"。

**修法**：让同花顺以**普通权限**运行（优于给 Python 提权：以后挂计划任务时
权限更小），并**退出精简模式**。

#### 2.2.3 打通权限后的实测（2026-08-21 下午）

预检除位数外全绿（`python=Medium / xiadan=Medium`，交易窗口正常显示）后：

| 能力 | 结果 |
|---|---|
| `easytrader.use('universal_client').connect(exe)` | ✅ |
| 左侧菜单导航（`查询[F4]` → 各页） | ✅ 可见 grid 随之切换 |
| `balance`（走 control_id，不经 grid） | ✅ 稳定，4 字段 `资金余额/可用金额/可取金额/总资产` |
| `position` / `today_entrusts` / `today_trades`（走 grid） | ❌ 见下 |

**只有第一次运行成功过一次**，读到 `today_entrusts` 2 行、12 列完整表头：
`委托时间、证券代码、证券名称、操作、备注、委托数量、成交数量、委托价格、
成交均价、撤消数量、合同编号、交易市场`。此后再没成功过。

**三种 grid 策略实测（各 3 轮 × 3 张表）**：

| 策略 | 表面成绩 | 真相 |
|---|---|---|
| `Xls`（Ctrl+S 导出文件） | **0/9**，`FileNotFoundError` | 有一次弹出的是 `提示: Begin failed!` 错误框而非保存框 |
| `Copy`（Ctrl+A/Ctrl+C） | 9/9 "成功" | **假的**，见下 |
| `WMCopy`（`post_message`） | 9/9 "成功" | **假的**，见下 |

⚠️ **最重要的一条：`Copy`/`WMCopy` 是静默失败的。**
它们不抛异常，直接返回空列表，看起来完全正常。用哨兵值验证才发现真相：
取表前把 `__QBG_SENTINEL__` 写进剪贴板，取表后**哨兵原封不动** ——
`Ctrl+A/Ctrl+C` 根本没执行，pandas 只是把陈旧内容解析成了 0 行。
即使用 `AttachThreadInput` 强制把交易窗口切到前台（验证过
`GetForegroundWindow()` 确实等于目标 hwnd）仍然如此。

> **这条直接决定 P9a 的实现方式**：取表的成功判据**不能是"没抛异常"**，
> 必须是肯定性校验 —— 写哨兵、取表、确认剪贴板/文件真的被更新了。
> 我自己写的基准测试就先被骗过一次，给了 `Copy`/`WMCopy` 9/9 的满分。

#### 2.2.4 根因：同花顺对「拷贝数据」本身有反爬验证码

上面一度把 32/64 位不匹配当成首要嫌疑，**那是错的**。用户看到客户端反复弹框
后定位到真正原因，抓到了弹窗结构：

```
#32770  416x242
  id=2393 Static  '检测到您正在拷贝数据，为保护您的账号数据安全，请'
  id=2394 Static  '先输入验证码：'
  id=2405 Static  (验证码图片)
  id=2404 Edit    (输入框)
  id=2406 Static  '验证码错误!!'
  id=1    Button  '确定'      id=2 Button '取消'
```

**触发条件是「拷贝数据」这个动作本身，与表是否为空无关。** 这个框是**模态**的，
一切都能解释了：

* `Copy` / `WMCopy`：模态框挡着，`Ctrl+A/Ctrl+C` 进不了 grid → 剪贴板保持原样 →
  pandas 把陈旧内容解析成 0 行 → **静默返回空列表，不抛异常**
* `Xls`：`Ctrl+S` 后 easytrader 以为弹出来的是保存框，把临时文件路径
  `set_edit_text` 进了**验证码输入框（2404）** → 于是有了 `验证码错误!!` →
  它自己点掉取消 → 文件从未生成 → `FileNotFoundError`
* 唯一成功的那一次，是验证码尚未触发的窗口期

**结论：验证码识别是 P9a 的硬前置，不是可选项。**

> 这里要公道地承认：THSAutoTrader **自带打包好的 Tesseract-OCR**，
> 而我在 §2.2.1 把这一点算成了它的减分项（500 MB 不可审计的二进制）。
> 就验证码这一件事而言，它确实是开箱即用的。选型结论不变
> （下单没有价格参数是硬伤），但这条得记上。

#### 2.2.5 打通（2026-08-21 傍晚）：easytrader 三个策略都不能直接用

选了 **ddddocr**（纯 pip、约 10 MB ONNX、专为四位扭曲验证码训练，
无需往系统装二进制）。装完发现**光换 OCR 后端不够** —— easytrader 自带的
三个 grid 策略在同花顺 9.60.61 上没有一个能用，而且原因各不相同：

| 策略 | 为什么不能用 |
|---|---|
| `Xls` | **不继承 `Copy`，零验证码处理**。`Ctrl+S` 后弹的是验证码框，它当成保存框，把临时文件路径 `set_edit_text` 进验证码输入框（2404）→ `验证码错误!!` → 自己点掉取消 → 文件从未生成 → `FileNotFoundError` |
| `Copy` | 有验证码分支，但用 `title_re="验证码"` 找控件，而 **pywinauto 的 `title_re` 是 `re.match`（锚定开头）**；本版文案是「先输入验证码：」，match 不上 → 分支不触发 → 读到陈旧剪贴板 → **静默返回空列表** |
| `WMCopy` | 继承 `Copy`，同一个检测缺陷 |

所以在 `tools/probe_ths.py` 里自己实现了 `CaptchaAwareCopy`（`--grid auto`，默认）。
四个必须自己做的点，每一条都是踩出来的：

1. **按可见性消歧 grid** —— 本机 `control_id=1047` 有 **6 个** `CVirtualGridCtrl`，
   恰好只有当前查询页那个可见。easytrader 原生的 `child_window(control_id=...)`
   在这里直接 `ElementAmbiguousError`。
2. **找验证码框不能用 `app.windows()`** —— 实测它**枚举不到**这个框（返回列表里
   只有主窗口），于是"等验证码"等多久都是白等。改用裸 `EnumWindows`，
   并按**控件结构**认（图片 2405 + 输入框 2404），不按文案。
3. **填验证码必须用 `type_keys`，不能用 `set_edit_text`。**
   实测对比：`set_edit_text` 走 `WM_SETTEXT`，输入框收下了却不认账，点确定弹窗不关；
   `type_keys` 发真键盘事件就能过。⚠️ **两种情况下 `window_text()` 回读都是空**，
   所以不能拿回读当校验。
4. **哨兵校验** —— 取表前往剪贴板写一个哨兵值，取完确认它真被覆盖了才算成功。
   没有这一步就无法区分"表是空的"和"复制根本没发生"。

**实测结果**：ddddocr 识别准确（抽查三张：`2603`/`9909`/`3169`，与图一致），
四张表全部取到，`Position` 需要的 7 个字段**全部对上**：

```
资金股票(持仓) 18 列：操作 序号 证券代码 证券名称 股票余额 可用余额 冻结数量
  成本价 市价 盈亏 盈亏比例(%) 当日盈亏 当日盈亏比(%) 市值 仓位占比(%)
  当日买入 当日卖出 交易市场
...
⚠️ **是「市值」不是「最新市值」** —— 别照抄 easytrader 文档或别的券商版本的列名。
⚠️ **`可用余额` 是 easytrader 相对截图 OCR 的真正增量**：T+1 闸需要它，
而手机截图里经常没有这一列。

**其他实测细节**：

* 反爬验证码是**异步**弹的，可能在取表调用返回后好几秒才冒出来；
  探针因此在收尾轮询 6 秒清理残留框。
* 同花顺的**营销提示框**（`现有成交价格预警服务`）也会不定时插进来，同样是模态的。
  它的按钮是「立即重启」「前往普通下单」「点击此处前去开通~」，**语义都不安全**，
  而且 id 分配和验证码框相反（验证码框 `id=2` 是『取消』，营销框 `id=2` 是『确定』）。
  所以清理一律用 `WM_CLOSE`，**认不出来的框绝不猜它的按钮**。
* 复制这一步本身不稳定（同一张空表连跑有时产出表头、有时纹丝不动），
  策略里重试 3 次；有哨兵兜底，重试不会把陈旧数据当成新数据。

**未排除但已降级的嫌疑**：32/64 位不匹配。pywinauto 仍会警告，
`SysTreeView32` 表项枚举返回空也仍是事实，但**不影响取表**，暂不处理。
⚠️ conda **已无 win-32 的 python 3.11**（Anaconda 砍了 32 位 Windows），
`CONDA_FORCE_32BIT` 那条常见建议行不通；真要建只能装 python.org 的 x86 安装包再 `venv`。

**还差一步**：当前是**全新空仓的模拟账户**，只验证了表头映射，
**取值**（数量、成本价、市值的格式与精度）要等账户里真有持仓后再跑一次。
...
### P9 — easytrader 接管持仓与下单

**状态**：2026-08-21 由"可选"上调为**计划中**（用户明确要求自动下单 +
自动持仓）。选型依据见 §2.2.1，风险纪律见 §8.7。

**运行位置**：**全部在 Windows 本机**。同花顺只有 Windows 客户端，
UI 自动化必须和它同机。2026-08-21 用户确认暂不上 Linux 云服务器，
一切以当前 Windows 机器为主 —— 所以本阶段**没有跨机通信问题**，
`easytrader.server` / `remoteclient` 用不上。

分三步走，**每一步都能独立停下来并保有价值**：

#### P9a — 只读持仓 ✅ 已实现（2026-08-21）

`portfolio/easytrader_src.py` 实现 `PositionSource` 协议，
`QBG_PORTFOLIO_SOURCE=easytrader` 时启用。

* 字段映射以 `tools/probe_ths.py` 探到的**真实表头**为准，不要猜
* **复用 P5 已有的六条校验**（`ocr_source.require_valid`）—— 数据来源换了，
  校验不能省；UI 自动化读错数字和 OCR 读错数字是同一类故障
* **失败降级到 `ocr` 源**，不抛给主流水线
* `Position.sellable_qty` 直接取"可用余额"列 —— 这是 easytrader 相对 OCR 的
  真正增量：截图里"可用"经常被省略，而 T+1 闸需要它

**收益**：消灭 §8.3 那条"OCR 读错数字 → 下错单"的最短资金错误传导路径。

**落地情况**：

| 文件 | 职责 |
|---|---|
| `src/qbg/portfolio/ths_client.py` | Windows/pywinauto 取表层。`CaptchaAwareCopy` + 验证码 + 弹窗清理，全部第三方依赖**函数内延迟导入**，所以 Linux/CI 上 import 不会失败 |
| `src/qbg/portfolio/easytrader_source.py` | 表头 → P5 payload 形状 → **复用 `ocr_source.validate_payload` 的六条校验**；表头缺列直接硬失败 |
| `src/qbg/portfolio/source.py` | `load_portfolio()` 工厂 + 降级，返回带来源和降级原因的 `LoadedPortfolio` |
| `tools/probe_ths.py` | 改为 **import** `ths_client`，不再各存一份 —— 否则"探针测通了"不代表生产路径也通 |

**顺带修掉一个既有的洞**：`daily_cycle` 在此之前是**硬编码** `ManualSource()` 的，
`QBG_PORTFOLIO_SOURCE` 配置项存在但没人读。之前没出问题只是因为 ocr 和 manual
恰好写同一份 CSV。

**降级必须可见**：日报新增「持仓来源」段，降级时打出醒目警告块。
静默降级比读失败更危险 —— 用过期持仓出的清单和正常清单长得一模一样。

**验收**：15 个离线测试（`tests/test_easytrader_source.py`，`read_tables` 整个被
替换，不需要开着同花顺）；真实客户端联调通过（读到 18 列表头、7 个必需字段全对上）。
**仍待完成**：账户里有持仓后再跑一次，验证**取值**的格式与精度。

#### P9b 前置：写入路径探测（2026-08-21，`tools/probe_ths_order.py`）

**在写 adapter 之前必须先钉死这两件事**，因为 easytrader 的写入路径上有两处
和取表层同源的问题。探测**不提交任何订单**，靠客户端的副作用做判断。

**雷一：`set_edit_text` 不被认账 —— 已实测引爆。**

easytrader 的 `_type_edit_control_keys` 默认走 `set_edit_text`（因为
`universal_clienttrader.login` 里设了 `_editor_need_type_keys = False`）。
在买入页（1032 代码 / 1033 价格 / 1034 数量）实测：

| 填值方式 | 控件回读 | 客户端回填的证券名称 | 可买股数 | 判定 |
|---|---|---|---|---|
| `set_edit_text`（默认） | `''` | `''` | `''` | ❌ 完全没反应 |
| `type_keys` | `''` | `'贵州茅台'` | `'100'` | ✅ 真的进去了 |

> ⚠️ **后果不是失败，是下错单**：easytrader 会「填」完价格数量（实际一个字
> 都没进），然后**照样点提交（1006）**。
>
> ⚠️ **不能用控件回读做校验**：两种方式 `window_text()` 都返回空字符串。
> 判据只能是**客户端的副作用** —— 代码填对后它会自动回填 `1036 证券名称`
> 和 `1018 可买(股)`。
>
> **修法已知**：adapter 必须调 `user.enable_type_keys_for_editor()`。

**雷二：弹窗能看见，但会被误处理 —— 形态和预期不同。**

原先担心 easytrader 看不见对话框（因为 `app.windows()` 实测枚举不到验证码框）。
**这个担心是错的**：`top_window()` 和 `app.windows()` 不是同一套机制，实测

    user.is_exist_pop_dialog()   -> True
    user._get_pop_dialog_title() -> '提示'
    user.app.top_window()        -> 正是验证码框的 hwnd

但风险换了个形态，而且更隐蔽 —— `PopDialogHandler.handle()`：

```python
if "提示" in title:
    content = self._extract_content()     # top_window().Static.window_text()
    self._submit_by_click()               # 直接点「确定」
    return {"message": content}
```

**反爬验证码框的标题恰好也是「提示」**。所以下单过程中若弹出验证码，
easytrader 会**不填验证码直接点确定**，并把 `'检测到您正在拷贝数据…'`
当成委托回执返回。而 `TradePopDialogHandler._extract_entrust_id` 会
`re.search(r"[\da-zA-Z]+", content)` 从任意文字里抠第一个字母数字串
**当委托编号** —— 凭空造出一个假委托号。

> **结论：P9b 必须自己接管弹窗处理**，按结构而不是按标题文字区分
> 「委托确认框」和「反爬验证码框」，就像 `ths_client._find_captcha_dialog`
> 那样。绝不能复用 easytrader 的 `TradePopDialogHandler`。

**雷三（新发现）：输入框清不干净 + 前台焦点 —— 都会静默出错。**

`tools/probe_ths_order.py` 第二轮实测又挖出两个坑，症状都和「不认账」一模一样：

| 坑 | 症状 | 修法 |
|---|---|---|
| `^a{DEL}` 清不掉输入框（该 Edit 不支持 Ctrl+A，DEL 只删一个字符） | 填完代码后客户端**自动回填市价 1272.90**，我们的 1213.97 被**追加**成 `1272.901213.97` | `{HOME}+{END}{DEL}`（Shift+End 选中再删） |
| 交易窗口丢前台焦点 | `set_focus()` 被前台锁静默拒绝 → 填了但客户端毫无反应 | 每步前 `force_foreground()` |

第一个坑还制造了一次**误导**：客户端弹「委托价格的小数部分应为 2 位，是否继续？」，
看着像价格格式写错，实际是没清干净导致有两个小数点。

**回读手段的边界**：`window_text()` 在这些框上**恒为空**，唯一可靠的回读是
**截图 + ddddocr**。但 ddddocr **不输出标点** —— `1213.97` 一律读成 `121397`。
我一度据此断定小数点没打进去，截图人眼看过才发现输入本来就是对的。
所以回读校验只能比**数字序列**：能抓住数字错了/多了/少了，
抓不住小数点位置。小数点由客户端自己的「小数部分应为 2 位」提示兜底 ——
**P9b 应把该提示当致命信号直接中止，而不是点「是」继续**。

**雷四（最隐蔽的一个）：`user.main` 可能根本不是交易窗口。**

easytrader 的 `connect()` 最后一行是 `self._main = self._app.top_window()` ——
`top_window()` 返回 **Z 序最上面**那个窗口。而这个客户端里有一个
**0x0 大小、却算「可见」的 `Internet Explorer_Hidden` 窗口**，它会抢到这个位置。
一旦抢到，后续所有 `child_window(...)` 都在错误的父窗口下查找：

```
ElementNotFoundError: {'control_id': 2322, 'class_name': 'ComboBox',
  'parent': <HwndElementInfo - '', Internet Explorer_Hidden, 266072>}
```

报错信息只说「找不到 control_id=2322」，**完全看不出根因是主窗口认错了**。
更麻烦的是它**间歇性发作**（取决于 connect 那一刻的 Z 序）—— 前几次能跑通
纯属运气。偶尔失败比一直失败难查得多。

**修法**：`ths_client.bind_main_window(user)`，connect 之后按窗口标题
（`网上股票交易系统5.0`）用裸 `EnumWindows` 重新钉死 `_main`。
`read_tables` 已内置调用，两个探针也都接上了。

> 这条影响的**不只是 P9b，也包括已经上线的 P9a** —— 之前没出事只是没抽到。

**输入路径最终验证通过**（`type_keys` + 正确清空 + 前台 + 主窗口钉死）：

```
[OK ] 代码: 期望 '600519'  实际 '600519'
[OK ] 价格: 期望 '121397'  实际 '121397'
[OK ] 数量: 期望 '100'     实际 '100'
```

#### 阶段一收尾：模拟账户实单验证通过（2026-08-21 21:31）

用户授权后在 `模拟炒股-****` 上挂了一单跌停价买单，**下出去 → 回读到 → 撤掉了**：

```
委托时间 21:31:30  证券代码 600519  证券名称 贵州茅台  操作 买入
备注 '未成交'  委托数量 100  成交数量 0  委托价格 1213.97
成交均价 0.0  撤消数量 0  合同编号 '6216979694'  交易市场 '上海Ａ股'
撤单后 → 备注 '全部撤单'   撤消数量 100
```

**委托价格回读为 `1213.97`** —— 小数点完好，输入路径彻底证实。

**`today_entrusts` 可用于回读校验**，字段齐全：
`委托时间 / 证券代码 / 证券名称 / 操作 / 备注 / 委托数量 / 成交数量 /
委托价格 / 成交均价 / 撤消数量 / 合同编号 / 交易市场`。
⚠️ 里面**混着整行空白的占位行**（本次撤单后 4 行里 3 行是空的），必须先过滤。

#### 雷五：**绝不能按 control id 认弹窗按钮**

这是本轮最危险的发现。三个实测到的弹窗，同一个 id 语义相反：

| 弹窗 | id=1 | id=2 | 其他 |
|---|---|---|---|
| 反爬验证码框 | **确定** | 取消 | — |
| 下单提示框 | **立即重启**(隐藏) | **确定** | 1008 查询当前委托、2392 点击此处不再提示 |
| 撤单确认框 | — | — | 6 是(&Y)、7 否(&N)、**1754 改单** |

探测脚本按「先试 id=6，再试 id=1」去点，在下单提示框上**点中了「立即重启」**，
只因它恰好 `visible=False` 才没出事。

更危险的是撤单确认框里的 **`1754 改单`**，可见文本写明是
`(撤单并以新的价格委托)` —— 误点会**直接下出一张新单**。

**正确规则**（`ths_client.find_dialog_button`）：

1. 只看 `visible=True` 的 Button
2. 文本先去掉 `&` 助记符和括号（实际文本是 `是(&Y)`，按 `"是"` 精确匹配会落空 ——
   第一轮探测就栽在这，弹窗被 WM_CLOSE 当成取消，订单根本没提交）
3. 走**白名单**（确定/是/OK），并把 `改单/立即重启/前往普通下单/前去开通/不再提示`
   显式**拉黑**
4. 认不出就 `WM_CLOSE`，绝不猜
...
* **三把锁照旧**：`QBG_MODE=LIVE` + `I_CONFIRM_REAL=1` + `allow_live_mode: true`，
  人手动改三处，任何自动化流程不得触碰
* **限价单**，价格由 `order_planner` 按涨跌停夹逼后给出，
  绝不使用市价/闪电下单
* **先卖后买**：移植 quant-trading 的 `_submit_settled` 思路 ——
  先提交 SELL、轮询到持仓变成预期数量、重读可用资金、再下 BUY。
  quant-trading 加这个是因为纸面账户现金曾被打到 −$24,322
* **提交后回读 `today_entrusts` 校验**（见 §8.7 纪律 2）
* **单次运行订单数上限**，超过就中止并告警

#### P9c — 编排接入

`daily_cycle` 在 LIVE 模式下走 `EasytraderAdapter`，
ADVISORY 模式仍只出清单。日报增加"计划 vs 实际委托"对账段。

**前提**：P9a 两周无异常 + P9b 在**模拟/小额**下验证过。
...
P1–P2 是工作量和风险的大头。**P0–P6 打通即可每天产出可执行的下单清单**（系统已经有用）；P7 提升选股质量；P8 加自我改进回路；P9 把人工执行换成自动执行（三小步，见上）。
...
~~easytrader 久未维护~~ —— **这条初版判断已于 2026-08-21 更正，见 §2.2.1**。
easytrader 2026-02 仍在提交，不是弃坑项目。

但真正的风险没变，只是换了个名字：**任何走 UI 自动化的方案都吃同花顺的界面布局**。
control_id（1032 代码框 / 1033 价格框 / 1034 数量框 / 1006 提交 / 1047 表格）
是硬编码的，同花顺大版本升级重排控件就会失效，而且**失效方式往往是静默的**——
读到空表、或者把数字打进错误的框。

三条纪律：

1. **只读先行**：先用 `portfolio/easytrader_src.py` 跑两周，确认表头字段和数值
   与人眼看到的一致，再谈下单。
2. **下单前必须回读校验**：提交后查 `today_entrusts`，确认委托的代码/方向/
   数量/价格与计划一致，不一致立即告警并停止后续订单。
3. **任何异常降级到 OCR/manual 源，不允许让主流水线崩掉。**

`tools/probe_ths.py` 是这条风险的常备探针：升级同花顺后先跑它，
再跑日流程。
