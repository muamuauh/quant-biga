...
.NOTES
    **为什么盘前也要跑预检**：复核要调 LLM 中转站，而那条链路走 Clash 代理。
    07:45 的预检任务本来就会开好代理，所以这里这次是**兜底** —— 预检任务被
    窗口闸跳过、或者它自己失败的那天，复核不该跟着一起哑掉。预检是幂等的
    （代理开着就不发热键、同花顺在跑就只报告），多跑一次没有副作用。
...
param(
    ...
    [string]$ScheduledAt   = "",
    # 30 分钟：复核实测 58 分钟，08:00 + 30 = 08:30 起跑，刚好赶在 09:30 之前
    # 写完缓存。**再晚就不该跑** —— 跑不完的话日流程照样退回现场复核，
    # 等于同一批票的 LLM 账单付两遍。setup_schedule.ps1 会把这个值写进任务动作。
...
# --- 过期不补跑 -------------------------------------------------------------
# 窗口比日流程短得多（30 vs 120）：盘前任务的全部价值在于**赶在 09:30 之前
# 写完缓存**。晚于 08:30 被唤起就来不及了，而"来不及"的代价不是零 ——
# 日流程照样退回现场复核，这批票的 LLM 账单等于付两遍。
...
# --- 预检（兜底）：复核要调 LLM，那条链路走代理 -----------------------------
# 正常情况下 07:45 的预检任务已经把代理开好了，这次只是确认一遍。
