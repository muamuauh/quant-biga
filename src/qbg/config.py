...
class Settings(BaseSettings):
    ...
    # 旧行为，打开是一个显式的部署决定。
    #
    # **2026-09-14 八项闸结论：不开。** scripts/29_trailing_gate.py，k=3、每 10 日、
    # keep_rank=15、20bp 滑点、10 个相位平均：
    #
    #                          388 日窗口（生产）          630 日窗口（mid）
    #     arm / trail        Δ年化  Δ夏普  子区间  闸     Δ年化  Δ夏普  子区间  闸
    #     10% / 10%          +8.5p  +0.12   3/4   过     -3.7p  -0.03   0/4   不过
    #     15% /  5%          +2.8p  +0.07   2/4   过     -2.4p  +0.01   1/4   不过
    #
    # 短窗口上 16 组参数全部改善夏普和回撤，看着很美 —— **那是窗口运气**。
    # 长窗口上两组都掉在净年化和子区间两项，10/10 的四个子区间**无一例外**跑输
    # 不止盈。回撤只改善 1.5~2.8 个点，换来的是砍掉仍在涨的票。
    # 触发不是零（长窗口每相位 21~23 次），所以这是"做了事、做错了"，不是"没做事"。
    #
    # 和 quant-trading 2026-08-04 在美股上的结论一致（arm15/trail5 夏普 1.65→1.55），
    # 对动量型 top-K 它倾向于截断右尾。
    #
    # 另一个代价：10/10 在短窗口上让相位极差从 77 点涨到 104 点 —— 结果更依赖
    # "从哪天开始调仓"，而实盘只能落在其中一个相位上。
    #
    # **要开的话是风险偏好选择，不是分析支持**（和关择时同一种性质）：换"浮盈大了
    # 必落袋"的纪律感，知情接受每年少几个点。机制本身是完整的、测过的。
