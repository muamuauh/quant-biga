...
class Settings(BaseSettings):
    ...
    # 第三方数据源（akshare / baostock）单次 socket 操作的超时，秒。
    # **不是可选的调优项，是让降级链活过来的前提。** 2026-09-09 实测：日流程
    # 卡死 10 分钟，进程只有一条到 Clash 混合端口的 TCP 连接、CPU 累计 1.7 秒
    # —— 请求发出去了对端不回，而调用链上一个超时都没有。
    #
    # `fetch_index_constituents` 早就写好了 fail-soft 分支（拿不到就退回本地
    # universe 文件），但没有超时它**永远走不到**：那个 except 只挡得住
    # "立刻报错"，挡不住"永远不回"。同理 SourceChain 的"换下一个源"。
    #
    # 30 秒的依据：BaoStock 单只票的增量拉取实测在 0.3~1 秒量级，中证指数成分
    # 接口约 1 秒。30 秒已经是正常值的 30 倍，触发它基本等同于"对端挂了"。
    # 设 0 或负数 = 不设限（退回旧行为）。
    qbg_net_timeout_sec: float = 30.0
    ...
    #
    # **2026-09-14 之前回测引擎完全不模拟它**，0.50 是拍的。现在模拟了：
    # 不开止损时它几乎不触发（每相位 0~0.1 次 —— 调仓日之外没有卖出，现金涨不上来）；
    # 开 8% 止损之后每相位触发 6~10 次，是止损卖出后把钱重新投出去的那条路。
    # 0.30 在两个窗口上方向相反（止损 8% 时生产 −38p、长窗口 +19p），纯噪声，维持 0.50。
    qbg_rebalance_cash_trigger: float = 0.50
    # 移动止盈（qbg.risk.trailing）：浮盈峰值先达到 ARM 才上膛，之后从峰值回撤
    # TRAIL 就在**非调仓日**整仓卖出。任一为 0 = 关闭。
    #
    # **代码默认关闭**，部署值写 .env —— 和择时缓冲带同一个惯例：代码默认是
    # 旧行为，打开是一个显式的部署决定。
    #
    # **2026-09-14 八项闸结论：不开。** scripts/29_trailing_gate.py --arm 0.10 --trail 0.10，
    # k=3、每 10 日、keep_rank=15、20bp、现金触发 0.5、10 个相位平均：
    #
    #                  Δ年化   Δ夏普   Δ回撤   子区间   高原邻居夏普     闸
    #   388 日（生产）  +11.1p  +0.18   +7.5p    3/4    1.26~1.67      通过
    #   630 日（mid）   +13.7p  +0.20   -2.4p    2/4    0.96~1.13      **不过**（回撤、高原）
    #
    # 收益和夏普在两个窗口上都是正的，**没过的原因是长窗口回撤变差、且候选夏普 1.31
    # 比邻居高出一截** —— 站在尖峰上，不是平台上。短窗口还把相位极差从 75 点拉到 121 点。
    #
    # ⚠ **这组数字替换了当天早些时候的一版**（那版写的是长窗口年化 −3.7 点）。那版用的
    # 引擎有多日持仓复利 bug（连涨两个 10% 算成 +22.10%），而且没有模拟现金触发。
    # 修掉之后结论的方向（不开）没变，理由变了。
    #
    # 要开的话是风险偏好选择：收益大概率不亏，但长窗口回撤会更深，参数也不稳。
    qbg_trail_arm_pct: float = 0.0
    qbg_trail_pct: float = 0.0
    ...
    # 择时的**迟滞缓冲带**：risk-on 时跌破 sma×(1-band) 才转 off，risk-off 时
    # 升过 sma×(1+band) 才转 on。0 = 关闭（裸的 level >= sma，历史行为）。
    #
    # **默认 0 是"未验收"不是"最优"。** 2026-09-08 在修掉纳入前视的等权指数上
    # 实测（全样本 1620 日，翻仓含 10bp 滑点）：
    #
    #   SMA  缓冲   净年化    夏普    回撤     翻转   翻仓成本/年
    #   100  0%     +9.41%   0.63  -14.75%    75     1.77%   ← 现行
    #   100  3%    +11.31%   0.73  -15.39%    13     0.31%
    #   200  3%    +13.25%   0.81  -15.17%     7     0.17%   ← 网格最优
    #   一直满仓  +16.83%   0.90  -22.86%      0     0
    #
    # 病根是**抖动不是保守**：无缓冲时 1620 日里翻了 75 次、risk-off 中位只有
    # 3 天，那不是在躲熊市，是在均线上蹭。缩短 SMA 只会更糟（SMA=20 翻 203 次、
    # 成本 4.78%/年、夏普 0.26）。
    #
    # ⚠ **没有任何一格夏普超过"一直满仓"的 0.90。** 择时在这套数据上买到的
    # 只是回撤（-22.86% → -15.17%），代价是 3.6 个点年化。开不开是偏好问题，
    # 不是优化问题 —— 但现行配置是这笔交易里最差的版本。
    # 要改先过 tuning/ 八项闸，见 scripts/18_regime_recheck.py。
    qbg_market_sma_band: float = 0.0
    # 择时指数是否只算「当天确实在沪深300 里」的票（纳入日期见
    # data/universe.inclusion_dates）。0 = 关闭 = 拿今天的成分回看历史。
    #
    # **这个开关和上面的缓冲带是捆绑的，不能只开一个。** 2026-09-08 八项闸实测：
    #
    #   判据指数        缓冲 0% → 3%           闸
    #   老口径          夏普 0.75 → 0.72      ❌ net_return、subperiod(1/4)
    #   修正纳入前视    夏普 0.63 → 0.73      ✅ 八项全过
    #
    # 原因：老口径混着 2026 年才被纳入的票（当前 287 只里 136 只是 2020 年后
    # 才进的），曲线被这些"未来的赢家"抬得更平滑，抖动本来就少，缓冲带没什么
    # 可省 —— 于是只剩"晚一步进场"的代价。**病是被偏差掩盖的，不是不存在。**
    #
    # 单独打开这一个会让回测数字变难看（0.75 → 0.63），那不是变差，是原来那个
    # 数字本来就是偷看来的。要改两个一起改，见 scripts/19_band_gate.py。
    qbg_market_index_eligible: int = 0
    ...
    # ------------------------------------------------------------------
    # ------------------------------------------------------------------
    # 同花顺官方金融数据 API（富瑶 / fuyao.aicubes.cn）
    # ------------------------------------------------------------------
    # **它只有数据，没有下单。** 下单仍然走 easytrader + pywinauto 那套 UI
    # 自动化（§七），这个 API 替代不了它。
    #
    # 引进它只为一件事：**拿到当天的真实参考价。**
    #
    # 系统 09:30 下单，而 parquet 缓存里最新一根日线是**昨天**的，限价因此
    # 建立在一个隔夜之前的价格上。实测 0.2% 让价下 37.7% 的卖单会挂到市价
    # 错误的一侧（表见 risk_limits.yaml）。2026-09-11 就踩中：688041 昨收
    # 232.37、卖单限价 231.91，而当天集合竞价定在 **228.02**、全天区间
    # 225.37~231.50 —— 挂在市价上方，一整天没成交。
    # 若当时读到 228.02，按 0.2% 让价得 227.56，必成交。
    #
    # 所以这条链路是**可选增强**，任何失败都退回缓存收盘价（老行为）。
    # 留空 api_key 就等于没开。
    qbg_fuyao_base_url: str = "https://fuyao.aicubes.cn"
    fuyao_api_key: str = ""
    # 盘中快照 vs 集合竞价快照。09:25~09:30 之间集合竞价已定价而连续竞价还没
    # 开始，那一刻 auction 是唯一有当日价格的接口；09:30 之后两个都能用，
    # 快照的 last_price 更贴近此刻。取价顺序由 reference_prices() 决定。
    #
    # 单次最多 100 个 thscode（官方限制），分批发。
    qbg_fuyao_batch: int = 100
    ...
    # **影子模式：照常复核、照常记录评级，但不据此选股。** 目标持仓直接取模型排名。
    #
    # 2026-09-24 实盘归因（20 个交易日，60 个复核样本）：复核**拦掉**的票比**留下**的
    # 涨得多 —— 1 日超额 +0.66% vs −0.75%（差 −1.41 点），5 日 +4.57% vs +1.61%
    # （差 −2.96 点）。两个持有期方向一致。同期模型前 3 名本身是跑赢市场的
    # （5 日超额 +1.79%，Rank IC +0.029 不比回测差）—— 亏损出在复核这一层，不在选股。
    # 09-15~17 连续三天 0/5 全拦，满仓现金，而那 15 只被拦的票 5 日超额平均约 +8%。
    #
    # 为什么是影子而不是直接关：样本太小（同一只票反复出现、窗口互相重叠），
    # 不够格下"复核有害"的结论；而复核**从没有过任何回测证据**（LLM 太贵没法回测）。
    # 影子模式停掉可能的损害，同时继续积累"留下 vs 拦掉"的对照 —— 这个对照
    # 只需要行情数据就能算，不需要我们真的持有它们。
    #
    # 顺带修掉一个副作用：复核开着时选股只在候选那 5 只里挑，`QBG_KEEP_RANK=15`
    # 的迟滞实际上失效（排第 8 的持仓不在集合里，直接被卖）。回测是在全排名上
    # 跑迟滞的。影子模式下选股回到全排名，和回测一致。
    #
    # 代码默认 0（旧行为：复核真的拦截），部署通过 .env 显式打开。
    qbg_agents_shadow: int = 0
