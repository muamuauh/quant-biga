...
class Settings(BaseSettings):
    ...
    # 市场择时：股票池等权代理跌破 N 日均线则全部转现金。0 = 关闭择时。
    #
    # 2026-08-25 用**含成本**的压测重定（scripts/07_regime_stress.py，
    # 2020-01-02~2026-08-24，299 只，10bp 滑点 + 实际费率）：
    #
    #     SMA    年化     Sharpe   最大回撤   翻转   年化成本拖累
    #       0  16.85%    0.8985   -22.86%      0     0.00%   ← 不择时
    #      20   7.76%    0.5648   -18.03%    178     4.21%
    #      50   9.49%    0.6487   -19.05%    105     2.49%
    #     100  11.19%    0.7247   -18.61%     63     1.49%   ← 取这个
    #     150   9.57%    0.6270   -17.87%     47     1.12%
    #     200  11.66%    0.7330   -17.15%     43     1.02%
    #     300  11.03%    0.6996   -27.72%     40     0.95%
    #
    # 为什么从 20 改成 100：**上一版是在没有成本模型的压测里选的**
    # （那时 `returns = asset_returns * exposure`，翻转免费）。加上费用后
    # SMA=20 不但 Sharpe 最差，最大回撤也变差 —— 一年翻转 27 次、拖累 4.21%，
    # 出场更频繁却只换来账单。
    #
    # 为什么不取 argmax(200)：150 的 Sharpe 比 100 和 200 都低，300 的回撤
    # 甚至比不择时还烂 —— 这个曲面是**噪声不是高原**，挑最高点就是过拟合
    # （见 tuning/ 八闸里的「参数高原」一项）。100 在区间内部，且是 plan.md
    # 独立给出的原始默认值。
    #
    # 为什么不干脆关掉：不择时那一格年化最高，但它也是全表里**受生存者偏差
    # 污染最重的** —— 股票池是今天的沪深300，满仓幸存者复利六年半（§8.1）。
    # 回撤从 -22.86% 收到 -18.61% 是真的，那 5.7% 超额多半不是。
    #
    # ⚠ 三条限制，积累历史成分股快照后必须复验：
