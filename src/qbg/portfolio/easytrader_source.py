...
#
# **只取「可用金额」和「总资产」。** 另外两个都不能用：
#
#   可用金额 = 真正能下单的钱（已扣掉挂单冻结）。可负担性过滤必须用它。
#   资金余额 = ⚠ **不是现金**。2026-08-25 本机实测（模拟账户）：
#              资金余额 199,217.75，而总资产 199,470.46、持仓市值 135,884.00 ——
#              它接近总资产而不是现金，具体口径不明。
#              **别拿它当「含冻结的现金」用**，那会让恒等式差出十几万。
#   可取金额 = 可提现金额，受 T+1 交收影响，与下单能力无关。
#
# 冻结资金**不在这张表里**，得从今日委托推（见 `frozen_cash`）。
BALANCE_COLUMNS = {"总资产": "总资产", "可用金额": "可用资金"}


def frozen_cash(entrusts) -> float | None:
    """从今日委托推算**被挂单冻结的现金**。

    同花顺的资金表没有「冻结金额」这一列，但总资产恒等式必须知道它：

        总资产 = 可用金额 + 冻结资金 + 持仓市值

    2026-08-25 实测验证（模拟账户，一笔 300394 买入 200 股 @250 未成交）：
        13,570.46 + 50,016.00 + 135,884.00 = 199,470.46 = 总资产 ✓

    只有**未成交的买单**冻结现金 —— 卖单冻结的是股份，不是钱。
    未成交数量用 `委托数量 − 成交数量 − 撤消数量` 算，不去解析「备注」里的
    中文状态串（「未成交」「部分成交」「全部撤单」……），那个更容易随版本变。

    返回 `None` 表示**读不到委托表**，调用方据此把恒等式放宽 ——
    别默认成 0，那会把一笔正常挂单误判成账目错误。
    """
    if entrusts is None:
        return None
    total = 0.0
    for row in entrusts:
        if "买" not in str(row.get("操作") or ""):
            continue
        pending = (_as_number(row.get("委托数量")) - _as_number(row.get("成交数量"))
                   - _as_number(row.get("撤消数量")))
        if pending > 0:
            total += pending * _as_number(row.get("委托价格"))
    return total
...
def to_payload(tables: dict, *, asof: str | None = None) -> dict:
    ...
    # 冻结资金不在资金表里，从今日委托推。读不到委托表时是 None（未知），
    # 校验层据此放宽恒等式 —— 不能默认 0，那会把正常挂单误判成账目错误。
    payload["冻结资金"] = frozen_cash(tables.get("entrusts"))
    return payload
