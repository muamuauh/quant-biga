...
四段：
  ...
  C. 卖出路径**负向验证** —— 当天买入当天不可卖（T+1），所以卖 100 股应当被
     客户端拒绝。这一段验证的是：菜单能切到卖出页、`_assert_side` 通过、
     填单 + OCR 校验通过、**客户端的拒绝提示被我们的致命词表抓住**。
     这样既跑通了整条卖出链路，又不会真的卖掉刚建的仓。
  D. 清理 —— 撤掉任何遗留委托

**持仓会留着不卖。** 下一个交易日 `可用余额` 会变成 100，那时才能测真正的卖出。
"""
...
def phase_c_sell_path(user, code: str, qty: int) -> bool:
    """卖出路径负向验证：当天买入当天不可卖，预期被客户端拒绝。"""
    print("\n=== C. 卖出路径验证（预期被 T+1 拒绝）===")
    name, ref = reference_price(user, code.split(".")[0])
    price = round(ref * 0.98, 2)          # 略低于市价，若真能卖也不至于挂太远
    print(f"  卖单挂 {price:.2f}，数量 {qty}")
...
def main(argv=None) -> int:
    ...
    print("\n完成。持仓保留 —— 下一个交易日 sellable_qty 会变成可卖，")
    print("那时可以用 --skip-build 重跑，验证真正的卖出成交路径。")
    return 0
